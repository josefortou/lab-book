...
En este capítulo, aprendemos a crear varios tipos de gráficas usando la librería `ggplot2`, parte del `tidyverse`. La 
...
ggplot(data = oecd_sum, 
  ...
  geom_col() +
  guides(fill = FALSE) +
  labs(x = NULL, y = "Diferencia en años",
       title = "La brecha de la expectativa de vida en EEUU",
       subtitle = "Diferencia entre expectativa de vida promedio en EEUU y países OCDE, 1960-2015",
       caption = "Datos: OCDE. Basado en una gráfica de Christopher Ingraham publicada en el Washington Post, diciembre 27 de 2017.") +
  theme_classic()

ggplot(organdata, mapping = aes(x = roads, y = donors)) + 
  geom_point(color = "steelblue4") +
  annotate(geom = "text", x = 157, y = 33,
           label = "Países con tasas sorprendentemente altas", 
           hjust = 0) +
  annotate(geom = "rect", fill = "red", alpha = 0.2,
           xmin = 125, xmax = 155,
           ymin = 30, ymax = 35) +
  labs(title = "¿Por qué hay países donde hay más donantes de los esperados?", 
       x = "Tasa de accidentes viales/100k habs.",
       y = "Tasa de donantes/1M habs.",
       caption = "Fuente: OCDE.") +
  theme_classic()
```
...
Como vamos a trabajar extensamente con `ggplot2()` así como las librerías `dplyr()` y `tidyr()`, cargamos estas librerías:

```{r}
library(tidyverse)
```

En este instructivo, seguimos los capítulos 3-5 del libro de Kieran Healey, *Data Visualization: A Practical Introduction* (Princeton UP, 2018). Por eso, cargamos la librería `socviz` que acompaña al libro y que contiene algunos datos útiles para explorar. Además, vamos a usar los datos de `gapminder` para aprender a usar `ggplot2`. Luego introduciremos otras librerías opcionales.
...
theme_set(
  theme_classic(
    base_size = 14, ## cambiar tamaño de letra base
    base_family = "serif" ## cambiar tipo de fuente
  ) +
...
Otras librerías traen temas adicionales a las opciones de `ggplot2`. Por ejemplo, está la librería `ggthemes`:

```{r}
library(ggthemes)
```

La revista _The Economist_ es reconocida por su compromiso con el periodismo de datos, en particular la división Inteligence Unit. Podemos copiar el diseño de sus gráficas: 
...
Hay infinidad de temas disponibles en línea. Otras librerías son `ggthemr`. Algunas no se encuentran en CRAN. Por ejemplo, `ggtech` (descarga directa de GitHub) tiene temas que emulan el "look" de varias compañías de tecnología. Para descargar e instalar librerías directamente de GitHub, usamos `devtools::install_github()`, especificando el nombre de usuario y del repositorio:

```{r, eval = FALSE}
devtools::install_github("ricardo-bion/ggtech")
```

Otro ejemplo: el equipo de periodismo de datos de la BBC creó una biblioteca para crear y exportar gráficas de `ggplot2` en el estilo usado por esa agencia. Lo podemos instalar con:
...
gapminder %>%
  ...
  top_n(20, gdpPercap) %>% # solo las 20 observaciones con mayor PIB per cápita
