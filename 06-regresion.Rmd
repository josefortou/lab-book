# (PART) III: Análisis de Regresión {-}
...
- Principales funciones: `lm()`, `tidy()`, `glance()`, `augment()` `tab_model()`, `plot_model()`.
...
Supongamos que tenemos una pregunta sobre la relación causal entre el ingreso de un individuo y su nivel educativo. Nuestra hipótesis es que a mayor número de años de educación, mayor ingreso anual en promedio. Buscamos y encontramos datos al respecto, los cargamos en R, reorganizamos un poco y miramos las primeras filas de la base de datos.
...
Una implementación moderna de este tipo de resúmenes está en la librería `gtsummary` y su función `tbl_summary`.^[Otra opción es la librería `skimr`.]

```{r summary}
gtsummary::tbl_summary(riverside)
```
...
Hasta ahora, hemos discutido brevemente cómo interpretar los coeficientes de un modelo de regresión múltiple con distintas especificaciones, utilizando la función `summary()` para ver estos resultados. Pero podemos tanto refinar un poco más la interpretación, como mejorar el "look" de los resultados. 
...
- Educación (variable continua): *ceteris paribus*, cada incremento de una unidad en la variable se asocia con un incremento de aproximadamente `r tidy(modelo_ingreso) %>% filter(term == "edu") %>% pull(estimate) %>% round()` unidades en la variable dependiente. En otras palabras, manteniendo las demás variables constantes, un año adicional de educación se asocia en promedio con un incremento de `r tidy(modelo_ingreso) %>% filter(term == "edu") %>% pull(estimate) %>% round()` USD en el ingreso anual.
- Experiencia (continua): *ceteris paribus*, cada incremento de de una unidad en la variable se asocia con un incremento de aproximadamente `r tidy(modelo_ingreso) %>% filter(term == "senior") %>% pull(estimate) %>% round() * 0.01` unidades en la variable dependiente. En otras palabras, manteniendo las demás variables constantes, un año adicional de experiencia laboral se asocia en promedio con un incremento de `r tidy(modelo_ingreso) %>% filter(term == "senior") %>% pull(estimate) %>% round() * 0.01` USD en el ingreso anual.
- Género (*dummy*): *ceteris paribus*, ser hombre se asocia con un incremento de `r tidy(modelo_ingreso) %>% filter(term == "genderHombre") %>% pull(estimate) %>% round()` unidades en la variable dependiente, comparado con la categoría de base (ser mujer). En otras palabras, manteniendo las demás variables constantes, el ingreso anual de un hombre es en promedio `r tidy(modelo_ingreso) %>% filter(term == "genderHombre") %>% pull(estimate) %>% round()` USD que el de una mujer.

Estos tres coeficientes son estadísticamente significativos ($p < 0.05$), lo cual indica que la probabilidad de observar estos valores si la hipótesis nula ($\beta = 0$) fuese cierta es menos del 5%.

Por otro lado, la función `glance()` nos da una mirada al ajuste del modelo en general. Entre otras estadísticas de "bondad del ajuste", el $R^2$ indica la proporción de la variación de $Y$ explicada por el modelo:

```{r mod-glance}
glance(modelo_ingreso)
```
...
Usemos nuestros datos "aumentados" para hacer unos diagnósticos básicos del ajuste y los supuestos de nuestro modelo de regresión lineal.
...
```{r}
modelo_aug %>%
  ggplot(aes(.resid, edu)) + 
  geom_point() +
  labs(x = "Educación (años)", y = "Residuos.")
modelo_aug %>%
  ggplot(aes(.resid, senior)) + 
  geom_point() +
...
Dos formas principales de presentar los resultados de una regresión: tablas y gráficas. Vamos a utilizar la librería `sjPlot` para ambos.
...
Visualizar los resultados de modelos, especialmente las predicciones e incertidumbre, puede ayudar mucho a entenderlos mejor y comunicar nuestros resultados. Vamos a usar la función `plot_model()` de `sjPlot`.
...
También podemos usar `plot_model()` para ver los "efectos" de distintas variables, entendidos como el cambio en la variable dependiente para distintos valores de las independientes. Para esto, especificamos `type = pred` y las variables independientes con `terms =`. Esta función mantiene constantes los factores (variables categóricas) en su categoría de referencia y las variables numéricas según su valor promedio. Aquí, vemos el valor esperado (predicción) de ingreso en todo el rango de la variable educación, controlando por género (mujer) y experiencia (media).

```{r plot-ef}
modelo_ingreso %>%
  plot_model(type = "pred", terms = "edu") +
  scale_y_continuous(labels = scales::dollar) +
...
Adicionalmente, podemos especificar dos variables independientes para visualizar aún más variación. Si las dos variables son numéricas, `plot_model()` automáticamente escoge tres valores del rango de la segunda variable que le pasamos (incluyendo la media) y grafica tres líneas.

```{r plot-ef2}
modelo_ingreso %>%
  plot_model(type = "pred", terms = c("edu", "senior")) +
  scale_y_continuous(labels = scales::dollar) +
  labs(title = "Valores esperados de ingreso para distintos valores de educación y experiencia",
       x = "Educación (años)", y = "Ingreso (USD)", color = "Experiencia (años)")
```
...
```{r plot-ef3}
modelo_ingreso %>%
  plot_model(type = "pred", terms = c("edu", "senior [5, 25]")) +
  scale_y_continuous(labels = scales::dollar) +
  labs(title = "Valores esperados de ingreso para distintos valores de educación y experiencia",
       x = "Educación (años)", y = "Ingreso (USD)", color = "Experiencia (años)")
```

En cambio, si una de las variables es categórica (y definida como factor en R), `plot_model()` grafica una línea para cada categoría. Así, vemos más claramente la diferencia en ingreso para hombres y mujeres, por ejemplo. Además, evidencia que incluir una variable categórica esencialmente crea un intercepto distinto para cada grupo, sin cambiar la pendiente (el coeficiente).

```{r plot-ef4}
modelo_ingreso %>%
  plot_model(type = "pred", terms = c("edu", "gender")) +
  scale_y_continuous(labels = scales::dollar) +
  labs(title = "Valores esperados de ingreso para distintos valores de educación y género",
...
```{r plot-ef5}
modelo_ingreso %>%
  plot_model(type = "pred", terms = c("gender", "edu [5, 15, 25]")) +
  scale_y_continuous(labels = scales::dollar) +
  labs(title = "Valores esperados de ingreso para distintos valores de género y educación",
       x = "Género", y = "Ingreso (USD)", color = "Educación (años)")
```

```{r plot-ef6}
modelo_ingreso %>%
  plot_model(type = "eff", terms = c("senior", "edu [5, 25]")) +
  scale_y_continuous(labels = scales::dollar) +
  labs(title = "Valores esperados de ingreso para distintos valores de educación y experiencia",
       x = "Experiencia (años, log)", y = "Ingreso (USD)", color = "Educación (años)")
```
...
- Variables dependientes categóricas: regresión logística y modelos por máxima verosimilitud (MLE).
