...
Para efectos de nuestra discusión, una base de datos es una recopilación de información en un formato de filas (que representan casos u observaciones) y columnas (que representan variables o características de esos casos). Las celdas contienen información sobre características de cada caso. La mayor parte de los análisis estadísticos se realizan al hacer operaciones o aplicar funciones a bases de datos.
...
```{r}
ggplot(data = economics, aes(x = date, y = unemploy/pop)) + # variables a usar
  geom_line(color = "darkred") # tipo de gráfica: línea
```

Otras librerías vienen con sus propias bases de datos. Aquí cargamos datos de la librería `gapminder`, que se deriva del trabajo educativo de la [Gapminder Foundation](https://www.gapminder.org/).

```{r}
library(gapminder)
```

Etsa librería contiene una base de datos llamada `gapminder` también. Veamos 10 observaciones aleatorias de esta base de datos, usando la función `sample_n()`.
...
```{r}
ggplot(data = gapminder, aes(x = gdpPercap, y = lifeExp)) + # variables
  geom_point(alpha = 0.5, color = "darkblue") # tipo de gráfica, puntos con con opacidad 50%
```

Parece que hay una relación positiva, pero no estrictamente lineal. Más bien, puede ser de tipo logarítmico, indicando rendimientos decrecientes. Nuevamente, podemos hacer la transformación de la variable `gdpPercap` directamente en la función `ggplot()`.

```{r}
ggplot(data = gapminder, aes(x = log(gdpPercap), y = lifeExp)) + # variables
  geom_point(alpha = 0.5, color = "darkgreen") # tipo de gráfica, puntos con con opacidad 50%
```

Por otro lado, si nos interesa ver la frecuencia de una variable categórica, podríamos contar cuántas observaciones (países-año) hay por continente usando la función `count()`, algo similar a lo que produce `table()`:
...
Es más frecuente que estemos interesados en trabajar con otros datos que recogimos o descargamos de otras fuentes. En ese caso, debemos empezar por subirlos a RStudio Cloud y ubicarlos en una carpeta dentro del proyecto.[^1] Si queremos ver qué archivos ya hay en una carpeta, podemos usar la función `list.files()`, dándole la dirección de la carpeta:

[^1]: Vamos al panel inferior derecho, pestaña `Files`, click en `Upload` y seguimos las indicaciones.

```{r}
list.files("data/")
```

Vemos que en esta carpeta ya hay varios archivos. Dependiendo del tipo de archivo (Excel, valores separados por comas, Stata, etc.) utilizamos una función distinta. Las librerías `readr`, `readxl` y `haven` del `tidyverse` son nuestras amigas.

Carguemos los datos del proyecto [Polity IV](http://www.systemicpeace.org/inscrdata.html) sobre democracia institucional. Como están en formato Excel (`.xls`), usamos `read_excel()` de la librería `readxl` y le asignamos un nombre al objeto (los datos) que estamos creando.[^2] 

[^2]: También los podemos cargar usando los menús del programa; el asistente para cargar datos en RStudio (pestaña `Environment`, opción `Import Dataset`) es bastante bueno y nos arroja código que después podemos copiar en un Rmarkdown o Rscript para replicar nuestros análisis. Para confirmar que todo está bien, revisemos las primeras filas del objeto que creamos:
...
Parece que todo está bien. Miremos la distribución de la variable `polity2`, el popular indicador combinado de democracia de Polity IV. Aquí volvemos a usar la librería `ggplot2` en vez de las funciones de "base R" (como `plot()` y `barplot()`):

```{r}
ggplot(data = polity4, aes(x = polity2)) + # variables
  geom_histogram(binwidth = 1) # tipo de gráfica y ancho de las barras
```
...
Así mismo, podemos hacer una selección según valores de más de una variable. Nos valemos de otros operadores, como `&` y `|`. El operador `&` es el booleano "y" (AND). Con este, podemos seleccionar observaciones que cumplan dos condiciones simultáneamente (año 2007 y expectativa de vida por encima de los 60 años)

```{r}
head(filter(gapminder, year == 2007 & lifeExp > 60))
```

`filter()` (y `dplyr` en general) nos permite reemplazar los `&` por `,`:
...
```{r}
sample_n(filter(gapminder, continent %in% c("Europe", "Americas", "Oceania")), 5) 
```
...
```{r}
sample_n(filter(gapminder, continent != "Asia"), 5)
```

Dos funciones -`top_n()` y `top_frac`-nos permiten seleccionar las primeras observaciones cuando ordenamos los datos según una variable (como cuando hacemos "Sort" u "Ordenar" en Excel). Por ejemplo, aquí seleccionamos el "top 5" de países según PIB per cápita:
...
Además, si guardamos el objeto como un archivo `.csv` o `.xlsx` por ejemplo y lo cargamos después, podemos "saltarnos" muchas líneas de código. Si queremos guardar un archivo de Excel, usamos la función `write_xlsx()` de `writexl`.
...
Podemos usar el operador `:` para seleccionar un rango de variables (desde `year` hasta `pop`):
...
La función `ends_with()` hace lo opuesto: las que terminan.
...
Podemos renombrar columnas dentro de `select()`, pero esto elimina todas las variables no renombradas.

```{r}
select(gapminder, pib_percap = gdpPercap)
```

En cambio, `rename()` nos permite renombrar y mantener las demás:

```{r}
rename(
  gapminder, 
  # nombre_nuevo = nombre_viejo
  continente = continent, ano = year, exp_vida = lifeExp, pob = pop, pib_percap = gdpPercap
)
```
...
Otra forma de hacer esto es con la función `cut()`. A continuación, la utilizamos para partir la variable `polity2` en 5 grupos o categorías. 

```{r}
polity4 <- mutate( 
  polity4, 
  regimen_cut = cut(polity2, 5)
)
count(polity4, regimen_cut)
```

Si anidamos esta operación dentro de `factor()` podemos darle nombres a las categorías con el argumento `labels =`:
...
polity4 <- mutate( 
  ...
  regimen_cut = factor(
    cut(polity2, 5),
    labels = c("bajo", "med-bajo", "medio", "med-alto", "alto")
  )
)
count(polity4, regimen_cut)
```
...
polity4 <- mutate(
  polity4, 
  regimen = fct_relevel(regimen, c("autocracia", "anocracia", "democracia"))
)
count(polity4, regimen)
```
...
El operador `%>%` (*pipe*, tubo o o tubería) sirve para simplificar nuestro código. Viene de la librería `magrittr` (¿sí agarran la referencia?) y es usado extensamente en el `tidyverse`. Para entender la utilidad de los *pipes*, comparemos distintas formas de usar varias funciones al mismo tiempo:
...
```{r}
c(1, 1, 2, 3, 5, 8, 13, 21) %>% mean() %>% round() # tomar el vector, encontrar la media, redondear
```

Pueden usarse como "tuberías" que conectan varias funciones, como `filter()`, `select()` y `arrange()`, cada una construyendo sobre los resultados que arroja la anterior:

```{r}
gapminder %>% # tomar gapminder
  filter(year == 2007, continent == "Americas") %>% # filtrar por valores de year y continent
  select(country, gdpPercap) %>%  # seleccionar solo las variables country y gdpPercap
  arrange(desc(gdpPercap)) %>% # ordenar descendente segun gdpPercap
  head() # ver las primeras filas
```

Con tuberías más largas, podemos responder a preguntas interesantes y comparaciones relevantes de forma más eficiente. Por ejemplo, digamos que queremos encontrar la media continental del PIB per cápita en el ultimo ano para el que tenemos información, porque estamos interesados en explorar la variación espacial en la riqueza de las naciones. ¿Cómo lo hacemos? ¡Pues armamos una tubería con `group_by()` y `summarize()`! Comparar medidas entre dos grupos nos permite ver la relación entre una variable numérica y una categórica.
