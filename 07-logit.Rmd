...
¿Por qué algunos legisladores votan a favor de un proyecto de ley? ¿Cómo se relaciona la identificación partidista con la probabilidad de que un legislador vote "sí"? Utilicemos datos para intentar responder estas preguntas.
...
nafta <- read_dta("data/nafta.dta") %>%
  ...
  mutate(
    ...
    democrat = factor(democrat, levels = c(0, 1), labels = c("Rep.", "Dem."))
  )
```

Además de la variable de voto a favor/en contra (`vote` que toma los valores "Yes"/"No"), tenemos tres variables más:
...
- `cope93`: indicador COPE de posiciones pro-sindicato (0=anti-sindicatos, 100=pro-sindicatos).

Veamos cuántos representantes votaron sí o no:
...
- Mayores valores COPE se asocian con una menor probabilidad de votar por NAFTA.
- El efecto de COPE sobre la probabilidad de votar por NAFTA depende de la afiliación partidista: el efecto será mayor para Demócratas.
...
plot_model(
  modelo_int,
  type = "int",
  title = "Probabilidad esperada de votar por NAFTA",
  legend.title = "Partido"
)
```
