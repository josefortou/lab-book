...
En ocasiones, convertimos los resultados de estas operaciones en objetos de R al asignarles un nombre con el operador de asignación `<-`:
...
Con elementos tan sencillos como estos y otros que veremos más adelante, eventualmente exploramos datos y construimos modelos estadísticos. En esta breve introducción[^1], discutimos cómo trabajar con R usando RStudio en su versión web: RStudio Cloud. Nuestro principal objetivo es tener la bases para, más adelante, hacer análisis de datos exploratorio (con gráficas y tablas) y análisis estadístico multivariado.  

[^1]: El primer borrador de este texto tomaba elementos prestados de [Quick Tips for R and RMarkdown](https://drive.google.com/file/d/1W8NxN-lfHvDHHJWMBYd8qXQUqefP6Ek_/view).

## RStudio

Primero, hay que entender cómo vamos a usar R a través de RStudio (y más específicamente, RStudio Cloud). [RStudio](https://rstudio.com/) es una IDE (un *integrated development environment* o entorno de desarrollo integrado) que funciona como una interfaz gráfica de usuario (o GUI) para facilitar el uso del lenguaje R y sus capacidades. RStudio Cloud es la versión web de RStudio.

Los elementos centrales del programa RStudio -además de la barra de menús, que comparte con muchos otros programas de computador- son cuatro paneles que vemos en la ventana principal. El flujo de trabajo en RStudio sigue lo que hacemos en estos paneles:

1. Escribimos código, seudo-código, comentarios y hasta texto en el script (archivo `.R`) o RMarkdown (archivo `.Rmd`) (por defecto, el panel superior-izquierdo). En una sesión nueva de RStudio, este panel puede no aparecer. Lo activamos creando un archivo Rscript o RMarkdown nuevo en el menú `File`.
2. Ejecutamos (corremos, compilamos) el código en la consola (panel inferior-izquierdo). El símbolo `>` y una línea vacía nos indica que R está listo para ejecutar un comando. En la consola también vemos los resultados de nuestro código. Aquí también creamos objetos de R nuevos al asignarle nombre al resultado de una operación. Ejecutamos código de una de las siguientes maneras:
  - Escribimos código en la consola y presionamos la tecla `Enter`.
  ...
  - Hacemos clic en el botón `Run` ![clear](images/icons_rstudio/run_icon.png), ejecutando un bloque continuo de código.
3. Frecuentemente creamos *objetos* de R tales como bases de datos, variables, tablas, resultados en el `Environment` o ambiente de la sesión de RStudio (panel superior-derecho). Hacemos esto ejecutando código en la consola o usando menús y asignándole un nombre al resultado; por ejemplo, una base de datos se puede cargar con código o en `File > Import Dataset`. Todos los objetos que están en el ambiente quedan disponibles para usarse en el código. Podemos borrar *todos* los elementos haciendo clic en `Clear` ![clear](images/icons_rstudio/clear_icon.png). También desaparecen cuando cerramos la sesión, pero podemos guardarlos como archivos para usar después. Así mismo, el script o RMarkdown nos permite volver a construir objetos.
...
Todo proyecto en R empieza con un documento que contiene el código que vamos a ejecutar. Estos documentos son principalmente de dos tipos: Rscript o RMarkdown.

### Rscripts

Un script de R (termina en `.R` ![rmd](images/icons_rstudio/rscript_icon.png)) contiene *código* y todo lo que está en el documento será ejecutado como tal. Si queremos que algo no sea ejecutado como código, insertamos el símbolo `#` adelante (`#comentarios`) y todo lo que sigue en esa línea queda como comentario. 

Como señalamos arriba, podemos ejecutar líneas o pedazos individuales de código en un script seleccionando lo que queremos ejecutar y presionando `Ctrl + Enter` o el botón `Run` ![run](images/icons_rstudio/run_icon.png). Un Rscript nos permite tener un registro de nuestro trabajo, incluyendo resultados, gráficas, experimentos fallidos y, con el uso de comentarios (`#`), un registro de nuestro proceso analítico. 

### RMarkdown

Un documento RMarkdown (termina en .Rmd ![rmd](images/icons_rstudio/rmd_icon.png)) contiene *código y texto*, con el fin de producir un documento final legible por seres humanos (o sea, no sólo código). El código, escrito en R, está en "chunks", o bloques, delimitados claramente con `` ```{r} `` y `` ``` ``. Agregamos bloques con el botón ![clear](images/icons_rstudio/newchunk_icon.png) o el atajo `Ctrl + Alt + I`. A continuación, vemos un bloque de código:

```{r}
9^2
```

El texto en un RMarkdown usa trucos del lenguaje Markdown para darle formato a un documento de salida final (por ejemplo, esta guía está escrita en RMarkdown y produce un archivo `.html`). Este formato es ideal para realizar informes y talleres o artículos y hasta libros - puede producir documentos en formato `.html`, `.pdf` y `.docx`. Para crear archivos PDF, RMarkdown requiere tener LaTeX (un programa de composición de textos académicos y técnicos) instalado en el computador. Pero esto no es necesario si trabajamos en RStudio Cloud; solo requiere librerías de R como `knitr` y `rmarkdown`. Pronto hablaremos sobre librerías.
...
Cuando creamos un nuevo RMarkdown en RStudio (`File > New file > RMarkdown` o haciendo clic en el ícono ![newfile](images/icons_rstudio/newfile_icon.png)) podemos seleccionar el output deseado y a mismo tiempo llenar los campos básicos del YAML (`title:`, `author:` y `output:`). En la parte superior de este archivo RMarkdown pueden ver otro ejemplo de un YAML simple, pero que utiliza la librería `rmdformats` para producir un documento estéticamente agradable.

#### Ejecutar código

En RMarkdown también podemos ejecutar código de distintas maneras:
- Líneas de código individuales con `Ctrl + Enter`.
...
- Usando `Run all chunks above` ![runabove](images/icons_rstudio/runabove_icon.png), R ejecuta todos los bloques anteriores.
...
Hay muchas otras funciones, tales como `data.frame`, `head`, `str`,  `tibble` y `ggplot` que veremos hoy. Casi todo lo que hacemos en R es a través de funciones.[^2] Muchas de estas vienen incluidas en R (decimos que son parte de "base R"), pero otras vienen de librerías adicionales.
...
Si R les dice que hay un error, hay un error. La descripción del error aparece en la consola. Estas descripciones pueden ser crípticas, pero intenten buscarlas en Google - ¡seguro no son los primeros! StackOverflow es otro gran recurso y muchos resultados de Google los llevarán allá. Finalmente, en la consola, pueden ejecutar `?` seguido del nombre de la función que provocó el error para abrir el manual de usuario de la función (por ejemplo, `?mean`).

RStudio avisa (con una $\times$ en la margen izquierda del script) si hay un error de sintaxis en el código (por ejemplo, si falta un paréntesis o unas comillas) incluso antes de que lo corran. No es infalible, pero ayuda.

La principal fuente de errores en R son los errores de tipografía o escribir algo mal, como el nombre de un archivo, una función o de otro objeto en R. Recuerden que R es sensible a las mayúsculas, a los acentos y otros caracteres especiales y a la puntuación, así que debemos revisar bien que estamos escribiendo `View()` y no `view()` o `mis_datos` en vez de `MIS_DATOS`.

Otra error común es olvidar cerrar los paréntesis o comillas. Por ejemplo, `print(“Hello, world!` (sin comillas ni paréntesis de cierre) hace que R crea que el comando aún no está terminado. En este último caso, la última línea de la consola mostrará un símbolo `+` en vez del `>` habitual. Cancelen esto con la tecla `Esc` y revisen el código.

Los mensajes de error frecuentemente tienen tres partes. Por ejemplo, correr `mean(z)` sin haber definido `z` previamente arroja el mensaje: `“Error in mean(z) : object 'z' not found”`. Por partes, esto nos dice que:
...
Otras cosas importantes para tener en cuenta a la hora de "tejer" un RMarkdown:

- A veces copiamos código de un Rscript o de la consola a un RMarkdown. En esos caso, hay que tener cuidado de no pegar resultados de la consola o elementos como `>` (indica que R está listo para ejecutar un comando) y `+` (indica que R cree que el último pedazo de código continúa), porque arrojan errores.
